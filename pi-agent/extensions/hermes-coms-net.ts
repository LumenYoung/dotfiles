...
export default function (pi: ExtensionAPI) {
	...
	function handleInboundPrompt(data: any): void {
		...
		let branchCursor = 0;
		try {
			branchCursor = currentCtx?.sessionManager?.getBranch?.()?.length ?? 0;
		} catch {
			branchCursor = 0;
		}
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		// Walk only branch entries added after this inbound prompt was queued.
		// Startup/system-prompt replies and older assistant messages can otherwise
		// be mistaken for the answer to the current coms-net request.
		let lastAssistantText = "";
		const branch = ctx.sessionManager.getBranch();
		for (const entry of branch.slice(inbound.branch_cursor)) {

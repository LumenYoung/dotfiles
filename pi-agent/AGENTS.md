...
For simple localized edits, the main agent owns the change: retrieve the latest note first, prefer narrow `iwe_query` block updates with exact selectors, `expect` guards, and `dry_run`, then retrieve the result to verify it.

For substantial page changes beyond a simple block replacement—such as whole-page rewrites, multi-section revisions, restructuring, consolidation, or coordinated edits across pages—delegate to `iwe-page-editor` in foreground/blocking mode with forked context. Give it the overall intent and relevant keys rather than a fully scripted edit sequence. The editor owns the delegated IWE changes for that run; do not edit the same pages concurrently. After it returns, retrieve and review every changed page yourself rather than requiring the editor to paste the full results into its handoff.
